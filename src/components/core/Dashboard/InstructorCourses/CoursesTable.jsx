...
export default function CourseTable({ courses, setCourses }) {
  ...
  return (
    <>
      <Table className="rounded-xl border border-richblack-800">
        <Thead>
          <Tr className="flex gap-x-10 rounded-t-md border-b-richblack-800 px-6 py-2">
            <Th className="flex-1 text-left rounded-t-md border-b-richblack-800 text-richblack-100">
              Courses
            </Th>
                  ...
                  <div className="flex flex-col justify-between">
                    ...
                    {course.status === COURSE_STATUS.DRAFT ? (
                      ...
                      <div className="flex w-fit flex-row items-center gap-2 rounded-full bg-richblack-700 px-2 py-[2px] text-[12px] font-medium text-yellow-100 ">
                        <div className="flex h-3 w-3 items-center justify-center rounded-full bg-yellow text-richblack-700">
                          <FaCheck size={8} />
                        </div>
                        Published
                      </div>
      ...
      {confirmationalModal && (
        <ConfirmationModal modalData={confirmationalModal} />
      )}
    </>
  );
}
